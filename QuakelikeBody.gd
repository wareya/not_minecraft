...
func find_real_collision(motion : Vector3, dot_motion : Vector3, infinite_inertia : bool = true) -> CollisionData:
    ...
    while true:
        ...
        if any and (motion - collider_velocity).dot(result.collision_normal) > 0.0 and (dot_motion).dot(result.collision_normal) > 0.0:
...
func my_move_and_collide(motion : Vector3, dot_motion : Vector3, infinite_inertia : bool = true, test_only : bool = false) -> CollisionData:
    ...
    var result = find_real_collision(test_motion, dot_motion, infinite_inertia)
    ...
    if result:
        if !test_only:
            ...
            if collision_margin > 0.0:
                var result_reject_a = find_real_collision(-marginal_motion, dot_motion, infinite_inertia)
                if result_reject_a:
                ...
                var result_reject_b = find_real_collision(normal_rejection, dot_motion, infinite_inertia)
                if result_reject_b:
...
func get_floor_normal():
    if floor_collision:
        return floor_collision.normal
    return null

func get_floor_velocity():
    if floor_collision:
        return floor_collision.collider_velocity
    return null
...
func collide_into_floor(motion):
    ...
    for i in range(max_iters):
        collision = my_move_and_collide(motion, motion)
        if collision == null:
            break
        else:
...
func move_and_collide_vertically(motion_y):
    var vec = Vector3(0, motion_y, 0)
    return my_move_and_collide(vec, vec)
...
func attempt_stair_step(motion, raw_velocity, started_on_ground, is_wall, fallback = false):
    ...
    var horizontal_contact = my_move_and_collide(motion, raw_velocity)
    ...
    var test_offset = 0.05 if upward_contact else 0.01
...
func unstuck(velocity):
    ...
    for dir in [Vector3.UP, Vector3.DOWN, Vector3.LEFT, Vector3.RIGHT, Vector3.FORWARD, Vector3.BACK]:
        var dist = 0.001
        var result = find_real_collision(dir*dist, dir*dist, true)
        if result and result.travel.length() > dist and result.travel.dot(dir) < 0.0:
            ...
            var result2 = find_real_collision(result.travel, result.travel, true)
...
func custom_move_and_slide(delta, velocity):
    ...
    var above_a_floor = collision_is_floor(find_real_collision(Vector3(0, -10, 0), Vector3(0, -10, 0)))
    if velocity.y <= 0:
        no_floor_check = false
    
    var use_collider_velocity = false
    ...
    for _i in range(max_iters):
        ...
        var collision = my_move_and_collide(delta_velocity, raw_velocity)
        if collision == null:
            break
        else:
            _iters_done += 1
